...
| `trigger_event_details_json` | JSONB         | Nullable                                          | Stores the initial parameters or payload that triggered the workflow (e.g., API request body). For example, this field would store the client-provided `X-Idempotency-Key` if the CPOA workflow was triggered by an operation that requires end-to-end idempotency. CPOA's main Celery tasks (e.g., `cpoa_orchestrate_podcast_task`) then use this client-provided key (or their own Celery task ID if no external key is given) as the `idempotency_key` for their *own* record in the shared `idempotency_keys` table, ensuring the entire CPOA-managed orchestration is idempotent. |
