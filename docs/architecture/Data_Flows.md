...
* **Data Exchanged & Formats:**
    ...
    2.  **APIGW -> CPOA:** Internal request (e.g., HTTP/gRPC call or message queue event) mirroring the frontend request, potentially with added context (e.g., authenticated user ID).
    3.  **CPOA -> TDA (Task):**
    ...
    12. **CPOA -> APIGW:** HTTP Response. Payload: JSON Array of `SnippetDataObjects`.
    13. **APIGW -> FEND:** HTTP Response. Payload: JSON Array of `SnippetDataObjects`.
...
* **Data Exchanged & Formats:**
    ...
    2.  **APIGW -> CPOA:** Internal request forwarding the above.
    3.  **CPOA (Initial Response Path) -> APIGW -> FEND:** HTTP Response (JSON): `{"status": "GENERATING", "stream_id": "unique_stream_id_for_client_to_connect_to", "estimated_wait_time_seconds": 15}`. (The `stream_id` is crucial for the client to connect for audio).
    4.  **CPOA -> WCHA (Task):**
...
* **Agent Task & Workflow State (Managed by CPOA):**
    * **Data:** Status of ongoing and completed orchestration workflows and individual agent tasks (task IDs, status, start/end times, input/output references, error details).
    * **Storage:** Persistent database, potentially part of CPOA's internal state management or a dedicated workflow engine's database (e.g., PostgreSQL,
        MongoDB).
    * **Format:** Structured data (JSON or tables).
* **Topic Cache:**
