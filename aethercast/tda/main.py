...
celery_app = create_celery_app('tda_tasks')
...
def setup_json_logging(flask_app):
    ...
    service_filter = ServiceNameFilter("tda")
...
def _save_topic_to_db(topic_object: dict):
    conn = None
    cursor = None
    try:
        conn = get_db_connection()
        ...
        sql_insert = """
            ...
            ON CONFLICT (id) DO UPDATE SET
                ...
                keywords = EXCLUDED.keywords, source_url = EXCLUDED.source_url, source_name = EXCLUDED.source_name,
                original_topic_details = EXCLUDED.original_topic_details,
                llm_model_used_for_snippet = EXCLUDED.llm_model_used_for_snippet,
                cover_art_prompt = EXCLUDED.cover_art_prompt, image_url = EXCLUDED.image_url,
                generation_timestamp = EXCLUDED.generation_timestamp,
                last_accessed_timestamp = EXCLUDED.last_accessed_timestamp,
                relevance_score = EXCLUDED.relevance_score;
        """
        params = (
            topic_id_str, DB_TYPE_TOPIC, topic_object.get("title_suggestion"), summary_to_save,
            keywords_to_save, source_url, source_name,
            json.dumps(topic_object.get("original_topic_details")) if topic_object.get("original_topic_details") else None,
            None, None, topic_object.get("image_url"), publication_date_to_save,
            last_accessed_ts_to_save, topic_object.get("relevance_score")
        )
        cursor.execute(sql_insert, params)
        conn.commit()
        app.logger.info(f"Saved/Replaced topic {topic_id_str} to PostgreSQL DB: {topic_object.get('title_suggestion')}")
    except psycopg2.Error as e:
        app.logger.error(f"PostgreSQL error saving topic {topic_id_str}: {e}", exc_info=True)
        if conn: conn.rollback()
    except Exception as e_unexp:
        app.logger.error(f"Unexpected error saving topic {topic_id_str} to PostgreSQL DB: {e_unexp}", exc_info=True)
        if conn: conn.rollback()
    finally:
        if cursor: cursor.close()
        if conn: conn.close()
...
def call_real_news_api(keywords: list[str] = None, categories: list[str] = None, language: str = None, country: str = None) -> list[dict]:
    if not tda_config.get("USE_REAL_NEWS_API"): return []
    ...
    if endpoint == "top-headlines":
        if categories: params["category"] = categories[0] if isinstance(categories, list) else categories
    ...
    headers = {"X-Api-Key": tda_config["TDA_NEWS_API_KEY"], "User-Agent": tda_config.get("TDA_NEWS_USER_AGENT", "AethercastTopicDiscovery/0.1")}
    ...
    try:
        ...
        for article in articles:
            ...
            topic_object = {
                "topic_id": str(uuid.uuid4()), "source_feed_name": SOURCE_FEED_NEWS_API,
                "title_suggestion": title, "summary": summary_text, "keywords": query_keywords_list,
                "potential_sources": [{"url": article_url, "title": title, "source_name": source_name}],
                "relevance_score": round(random.uniform(0.6, 0.9), 2), "publication_date": published_at,
                "category_suggestion": "News", "original_topic_details": article
            }
...
def fetch_news_from_newsapi_task(self, request_id_celery: str, keywords: list[str] = None, categories: list[str] = None, language: str = None, country: str = None, idempotency_key: Optional[str] = None, workflow_id: Optional[str] = None):
    ...
    task_name_for_idempotency = self.name
    log_extra_base = {"orig_req_id": request_id_celery, "task_id": task_log_id, "idempotency_key": idempotency_key, "workflow_id": workflow_id, "keywords": keywords}
    ...
    try:
        ...
        existing_record = check_idempotency_key(db_conn, idempotency_key, task_name_for_idempotency)
        if existing_record:
        ...
        raise self.retry(exc=e_req, countdown=10, max_retries=2, idempotency_key=idempotency_key, workflow_id=workflow_id)
...
SIMULATED_DATA_SOURCES = [{"source_name": "Tech Chronicle", "articles": [{"title": "The Future of AI in Personalized Medicine", "url": "http://example.com/ai-medicine", "keywords": ["AI", "Healthcare", "Personalized Medicine"], "publish_date": "2024-03-10T10:00:00Z"}, {"title": "Quantum Computing: Beyond the Hype", "url": "http://example.com/quantum-beyond-hype", "keywords": ["Quantum Computing", "Technology", "Innovation"], "publish_date": "2024-03-11T11:00:00Z"},]}]
...
def identify_topics_from_sources(query: str = None, limit: int = 5) -> list:
    ...
    for data_source in SIMULATED_DATA_SOURCES:
        for article in data_source["articles"]:
            all_articles.append({"title": article["title"], "url": article["url"], "source_name": data_source["source_name"], "keywords": article.get("keywords", []), "publish_date": article.get("publish_date", datetime.utcnow().isoformat())})
    for article in all_articles:
        ...
        topic_object = {"topic_id": str(uuid.uuid4()), "title_suggestion": article["title"], "summary": generate_summary_from_title(article["title"]), "keywords": combined_keywords, "potential_sources": [{"url": article["url"], "title": article["title"], "source_name": article["source_name"]}], "relevance_score": relevance, "publication_date": article.get("publish_date"), "category_suggestion": "General", "original_topic_details": article}
...
def discover_topics_task(self, request_id_main: str, query: Optional[str], limit: int, use_real_news_api_flag: bool, error_trigger: Optional[str] = None, idempotency_key: Optional[str] = None, workflow_id: Optional[str] = None):
    task_log_id = self.request.id
    log_extra_base = {"orig_req_id": request_id_main, "task_id": task_log_id, "idempotency_key": idempotency_key, "workflow_id": workflow_id, "query": query}
    ...
    try:
        db_conn = get_db_connection()
        db_conn.autocommit = False
        existing_record = check_idempotency_key(db_conn, idempotency_key, self.name)
        if existing_record:
            status = existing_record['status']
            locked_at = existing_record.get('locked_at')
            if status == tda_config['IDEMPOTENCY_STATUS_COMPLETED']:
        ...
        if use_real_news_api_flag:
            ...
            news_task = fetch_news_from_newsapi_task.delay(request_id_celery=self.request.id, keywords=request_keywords, language=tda_config.get("TDA_NEWS_DEFAULT_LANGUAGE"), idempotency_key=derived_news_key, workflow_id=workflow_id)
...
def discover_topics_async_endpoint():
    ...
    app.logger.info(f"Request {request_id_main}: Received async /discover_topics request.")
    ...
    idem_task_name_for_db = 'discover_topics_task'
    db_conn_http = None
    try:
        db_conn_http = get_db_connection()
        db_conn_http.autocommit = False
        existing_record = check_idempotency_key(db_conn_http, idempotency_key, idem_task_name_for_db)
        if existing_record:
            status = existing_record['status']
            locked_at = existing_record.get('locked_at')
    ...
    if error_trigger == "tda_endpoint_error":
    ...
    app.logger.info(f"Request {request_id_main}: Dispatching topic discovery to Celery task. Query: '{query}', Limit: {limit}, UseNewsAPI: {use_real_news_api_flag}, Idempotency-Key: {idempotency_key}")
    task = discover_topics_task.delay(request_id_main=request_id_main, query=query, limit=limit, use_real_news_api_flag=use_real_news_api_flag, error_trigger=error_trigger, idempotency_key=idempotency_key, workflow_id=workflow_id)
    app.logger.info(f"Request {request_id_main}: Topic discovery task {task.id} dispatched.")
    return flask.jsonify({"task_id": task.id, "status_url": f"/v1/tasks/{task.id}", "message": "Topic discovery task initiated. Poll task ID for results.", "idempotency_key_processed": idempotency_key}), 202

@app.route('/v1/tasks/<task_id>', methods=['GET'])
def get_tda_task_status(task_id: str):
    app.logger.info(f"Received request for TDA task status: {task_id}")
    ...
    if task_result.successful():
        ...
        if isinstance(task_output, dict) and task_output.get("status") == "PROCESSING_CONFLICT":
            return flask.jsonify(response_data), 409
...
if __name__ == "__main__":
    ...
    if not all(tda_config.get(k) for k in required_pg_vars):
