...
-   **Headers:**
    -   `X-Idempotency-Key` (string, Optional but Recommended): If provided, this key is used as the `request_id` for the Celery task, enabling idempotent execution. If not provided, AIMS will generate a unique `request_id`.
...
-   **Failed Response (200 OK - Task Failed in Celery):**
    ...
    ```json
    {
        ...
        "status": "FAILURE",
        "result": {
            "error": {"type": "task_failed_exception_type", "message": "Vertex AI API Error: ..."}
        }
    }
    ```
...
-   **Idempotency Key:** If an `X-Idempotency-Key` header is provided by the client, AIMS uses this value as the `request_id` for its Celery task. If not provided, AIMS generates a unique `request_id`. This `request_id` serves as the idempotency key for the Celery task.
-   **Pattern:** A two-phase pattern is employed:
    1.  **Check/Lock:** Before executing the core LLM call, the Celery task checks the `idempotency_keys` table using its `request_id`.
