...
def load_iga_configuration():
    ...
    iga_config['IGA_GCS_IMAGE_PREFIX'] = os.getenv("IGA_GCS_IMAGE_PREFIX", "images/iga/")
    ...
    if not iga_config['GCS_BUCKET_NAME']:
...
if iga_config.get('IGA_VERTEXAI_IMAGE_MODEL_ID'):
    try:
...
def _get_iga_db_connection():
    ...
    try:
        conn = psycopg2.connect(
            host=iga_config['POSTGRES_HOST'], port=iga_config['POSTGRES_PORT'],
            user=iga_config['POSTGRES_USER'], password=iga_config['POSTGRES_PASSWORD'],
            dbname=iga_config['POSTGRES_DB'], cursor_factory=RealDictCursor
        )
    ...
    try:
        with db_conn.cursor() as cur:
            ...
            if record:
                ...
                if isinstance(record.get('result_payload'), str): record['result_payload'] = json.loads(record['result_payload'])
                if isinstance(record.get('error_payload'), str): record['error_payload'] = json.loads(record['error_payload'])
                return dict(record)
...
def generate_image_async_endpoint():
    ...
    task = generate_image_vertex_ai_task.delay(
        ...
        model_id=model_id_to_use, gcs_bucket_name=iga_config['GCS_BUCKET_NAME'], gcs_image_prefix=iga_config['IGA_GCS_IMAGE_PREFIX'],
