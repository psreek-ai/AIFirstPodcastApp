...
-   **`GET /api/v1/snippets`**
    -   **Description:** Fetches dynamically generated podcast snippets via CPOA, suitable for a landing page. CPOA orchestrates topic discovery, snippet text generation, and image generation.
    -   **Query Parameters:** `limit` (optional, integer, default 5, max 20): Number of snippets.
    -   **Success Response (200 OK):** `{"snippets": [...], "source": "generation"}` (content from CPOA).
...
-   **Key Columns:** `id` (PK), `type` ('topic' or 'snippet'), `title`, `summary`, `keywords` (JSON), `source_url`, `source_name`, `original_topic_details` (JSON for snippets), `llm_model_used_for_snippet`, `cover_art_prompt`, `generation_timestamp`, `last_accessed_timestamp`, `relevance_score`.
