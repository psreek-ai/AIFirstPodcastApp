...
class TestAggregateDdgsHarvestResultsTask(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_wcha_config = self.wcha_config_patcher.start() # Assign to attribute for consistency
...
class TestWchaCeleryLogging(unittest.TestCase):
    def setUp(self):
        # This class now inherits from WchaCeleryTaskTestCaseBase,
        # so common setup (Celery eager, stdout capture, idempotency mocks) is done there.
        super().setUp() # Call base class setUp

        # Specific patches for this class, if any, would go here.
        # For TestWchaCeleryLogging, the primary purpose is log capture and task execution,
        # so most mocks are already handled by the base or applied per-test.

    # _get_json_logs can remain here or be moved to base if universally needed by all derived classes
    # For now, keeping it here as it's specific to log testing.
    def _get_json_logs(self):
        log_output = sys.stdout.getvalue()
        sys.stdout = io.StringIO() # Reset for next capture
        ...
        for line in log_output.strip().split('\n'):
            if line:
                try:
                    ...
                    # Using self.held_stdout for test debug prints to avoid interfering with capture
    ...
    def test_harvest_url_content_task_json_logging_success(self, mock_is_safe, mock_extract, mock_requests_get):
        ...
        celery_task_id_prefix = "celery_harvest_task_" # Prefix of self.request.id if it were a real Celery task

        # Patch the task's self.request.id
        ...
        # Example: Check a specific log message (e.g., successful extraction)
        success_log_found = False
        for log_entry in logs:
            self.assertEqual(log_entry.get('service_name'), 'wcha-service')
            self.assertEqual(log_entry.get('task_id'), task_request_id) # Original request_id passed to task
            self.assertIn('workflow_id', log_entry) # Should be 'N/A' or a value
            self.assertIn(log_entry.get('name'), ['aethercast.wcha.main']) # Logger name
            if "Trafilatura successfully extracted" in log_entry.get("message", ""):
                ...
                # The Celery task ID from self.request.id might be part of the message for some logs
                # e.g., "Celery Task celery_harvest_task_..."
...
class TestFetchNewsArticlesTask(unittest.TestCase):
    def setUp(self):
        super().setUp() # Call base class setUp

        # Specific patches for TestFetchNewsArticlesTask
    ...
    # tearDown is handled by base class if only stdout restoration is needed.

    def _run_task(self, request_id_override=None, topic="test topic", **kwargs):
        # Helper to run the task with mocked self.request
        ...
        with patch.object(wcha_main.fetch_news_articles_task, 'request', MagicMock(id=self.celery_task_id)):
            return wcha_main.fetch_news_articles_task(
                request_id=task_request_id_to_use, # This is the idempotency key
                topic=topic,
                **kwargs
            )
    ...
    def test_core_logic_news_api_returns_error_status(self):
        ...
        with self.assertRaises(requests.exceptions.HTTPError) as cm: # Task re-raises HTTPError
            self._run_task()
    ...
    def test_core_logic_requests_exception(self):
        ...
        with patch.object(wcha_main.fetch_news_articles_task, 'retry', side_effect=Exception("Retry limit reached")): # Mock retry to prevent actual retries
            with self.assertRaises(Exception) as cm: # Assuming retry will re-raise or task fails after retries
                 self._run_task()

        self.assertIn("Retry limit reached", str(cm.exception)) # Check for the mocked retry failure
        self.mock_update_record.assert_called_once_with(
            self.mock_db_conn, self.task_request_id, "wcha_fetch_news_articles_task", 'failed',
    ...
    @patch.dict(wcha_main.wcha_config, {"USE_REAL_NEWS_API": True, "TDA_NEWS_API_KEY": None}) # No API Key
    ...
    def test_idempotency_acquire_lock_fails(self):
        ...
        self.mock_acquire_lock.return_value = False # Lock acquisition fails
        ...
        self.mock_requests_get.assert_not_called()
        self.mock_update_record.assert_not_called() # Not called because lock failed before core logic
...
class TestHarvestUrlContentTask(unittest.TestCase):
    def setUp(self):
        super().setUp() # Call base class setUp

        # Specific patches for TestHarvestUrlContentTask
    ...
    def test_idempotency_acquire_lock_fails(self):
        ...
        self.assertEqual(result["status"], "error")
        self.assertEqual(result["message"], "Failed to acquire idempotency lock.")
        self.mock_is_url_safe.assert_not_called() # Should not proceed if lock fails
    ...
    def test_import_failure_requests(self):
        self.mock_check_idempotency.return_value = None
        self.mock_acquire_lock.return_value = True
        # is_url_safe is called before import checks in the task, so mock it
        self.mock_is_url_safe.return_value = (True, "URL is safe.")
    ...
    def test_requests_get_raises_exception(self):
        ...
        self.mock_acquire_lock.return_value = True
        self.mock_is_url_safe.return_value = (True, "URL is safe.")
        self.mock_requests_get.side_effect = requests.exceptions.Timeout("Connection timeout")
        ...
        self.mock_update_record.assert_called_once_with(
            self.mock_db_conn, self.task_request_id, "wcha_harvest_url_content_task", 'failed',
            error_payload={'error_type': 'Timeout', 'message': 'Connection timeout', 'url': self.test_url}
        )
...
class WchaCeleryTaskTestCaseBase(unittest.TestCase):
    def setUp(self):
        ...
        self.held_stderr = sys.stderr # In case stderr is also used by some debug/error paths
...
# Modify existing Celery task test classes to inherit from the base
class TestAggregateDdgsHarvestResultsTask(WchaCeleryTaskTestCaseBase): # Inherit from base
    def setUp(self):
        super().setUp() # Call base setUp
        # Specific patches for this class
        self.wcha_config_patcher = patch.dict(wcha_main.wcha_config, {'WCHA_MIN_CONTENT_LENGTH_FOR_AGGREGATION': 10})
        self.mock_wcha_config = self.wcha_config_patcher.start()
        self.addCleanup(self.wcha_config_patcher.stop)

    # ... rest of TestAggregateDdgsHarvestResultsTask methods remain unchanged
    @patch('aethercast.wcha.main.AsyncResult')
    def test_aggregation_all_sub_tasks_success(self, MockAsyncResult):
        self.mock_check_idempotency.return_value = None
        self.mock_acquire_lock.return_value = True
    ...
    def test_aggregation_content_too_short(self, MockAsyncResult):
        ...
        # self.mock_wcha_config is set in setUp for this class
        with patch.dict(self.mock_wcha_config, {'WCHA_MIN_CONTENT_LENGTH_FOR_AGGREGATION': 15}): # Modify the already patched config
            agg_task_id = "agg_content_short"
            mock_self = MagicMock(); mock_self.request.id = "celery_agg_short"
            result = wcha_main.aggregate_ddgs_harvest_results_task.__wrapped__(
                mock_self, agg_task_id, harvest_task_ids, "Content Too Short Topic"
...
class TestWchaCeleryLogging(WchaCeleryTaskTestCaseBase):
    # Methods from original TestWchaCeleryLogging
    def setUp(self):
        super().setUp()
